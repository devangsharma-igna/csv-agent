...
async def query(
    ...
    try:
        ...
        if context is None:
            ...
            log.info("phase=pre_context | cached context (%d columns)", len(context.get("columns", [])))
        ...
        if answer.get("wants_figure"):
            ...
            log.info("phase=pre_figure | skipped")
        ...
        raise HTTPException(
            status_code=410,
            detail={"error": "table_deleted", "table": e.table, "phase": e.phase, "message": str(e)},
        ) from e
    except MCPToolError as e:
        log.error("query ABORT (db_error) | %s", trunc(e.message, 400))
